...
- **Dune:** wallet query scales with market count. If resource cap hit, reduce `TOP_N_MARKETS` or check per-query credit limit in Dune account settings.
  - **Check pipeline key usage:** `POST https://api.dune.com/api/v1/usage` with `X-Dune-Api-Key` header — returns `credits_used` and `credits_included` for the billing period.
  - **Check MCP key usage:** `mcp__dune__getUsage` tool (no args) — reports the MCP account's credits separately from the pipeline key.
  - Two separate Dune accounts: pipeline key is in `.env` (DUNE_API_KEY), MCP key is configured in Claude Code settings. Check both independently.
